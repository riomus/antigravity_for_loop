...
When the task is fully complete and all tests pass, respond with exactly: {{completionPromise}}`;
    ...
    async checkQuotaExceeded() {
        ...
        try {
            this.log('[Ralph] Checking quota...');
            const result = await this.cdp.sendCommand('Runtime.evaluate', {
                expression: `
                    (function() {
                        const iframe = document.getElementById('antigravity.agentPanel') || document;
                        const doc = iframe.contentDocument || iframe.contentWindow?.document || document;
                        const elements = doc.querySelectorAll('.absolute');
                        // concatenated text of all the elements
                        const text = Array.from(elements).map(el => el.textContent).join(' ');
                        
                        // Look for "Model quota limit exceeded"
                        console.log(text);
                        if (text.includes('Model quota limit exceeded' || text.includes('Select another model') || text.includes('You have reached the quota limit for this model.')) ) {
                            // date extracted from text of the message that is like You have reached the quota limit for Gemini 3 Pro (High). You can resume using this model at 21.01.2026, 15:39:33. You can upgrade to the Google AI Ultra plan to receive the highest rate limits. 
                            const re = new RegExp("/(\d{2}\.\d{2}\.\d{4}),\s*(\d{2}:\d{2}:\d{2})/", "g");
                            const match = text.match(re);
                            console.log(match);
                            let date = match ? match[0] : null
                            if (!date){
                             // try to extract the date using text splits
                             const parts = text.split('You can resume using this model at ');
                             if (parts.length > 1){
                                date = parts[1].split(". ")[0].trim();
                             }
                            }
                            return { 
                                exceeded: true, 
            ...
            this.log(`[Ralph] Quota check result: ${JSON.stringify(result)}`);

            const value = result?.result?.value;
            if (value && value.exceeded) {
                let resumeDate = null;
                if (value.resumeDateStr) {
                    const m = value.resumeDateStr.match(/^(\d{2})\.(\d{2})\.(\d{4}),\s*(\d{2}):(\d{2}):(\d{2})$/);
                    if (!m) throw new Error("Bad datetime format");

                    const [, dd, mm, yyyy, HH, MM, SS] = m;

                    const dt = new Date(
                        Number(yyyy),
                        Number(mm) - 1,   // months are 0-based
                        Number(dd),
                        Number(HH),
                        Number(MM),
                        Number(SS)
    ...
    async manageQuota() {
        ...
        if (quota.exceeded) {
            ...
            while (this.isRunning) {
                ...
                let waitTime = diff || this.options.quotaCheckInterval;
                const minutes = Math.ceil(waitTime / 60000); // Minutes remaining
    ...
    async start() {
        ...
        try {
            ...
            for (this.currentIteration = 1; this.currentIteration <= this.options.maxIterations; this.currentIteration++) {
                ...
                // Post-Iteration Prompt (run after AI responds, before completion checks)
                ...
                if (aiResponse && aiResponse.includes(this.options.completionPromise) && aiResponse.length - this.options.completionPromise.length < 5) {
                    ...
                    if (this.options.checkPrompt) {
                        ...
                        const expectedPromise = this.options.successKeyword || 'PASS';
                        if (checkResponse && checkResponse.includes(expectedPromise)) {
