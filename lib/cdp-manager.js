            ...
            window.__antigravityForLoop = {
                ...
                injectPrompt: function(text) {
                    ...
                    try {
                        ...
                        // Check if it's a Lexical editor (needs special handling)
                        const isLexical = input.hasAttribute('data-lexical-editor');
                        const iframeDoc = input._iframeDoc;
                        ...
                        } else if (isLexical && iframeDoc) {
                            // Lexical editor - use execCommand for proper text insertion
                            console.log('[For Loop] Using execCommand for Lexical editor');
                            iframeDoc.execCommand('selectAll', false, null);
                            iframeDoc.execCommand('insertText', false, text);
                        } else if (input.getAttribute('contenteditable') === 'true') {
                            // Generic contenteditable div - try execCommand first
                        ...
                        console.log('[For Loop] Prompt injected successfully');
                        this.lastInjectionResult = { success: true, inputType: input.tagName, isLexical };
                        return true;
                    } catch (e) {
                ...
                isAcceptButton: function(el) {
                    ...
                    const acceptPatterns = ['accept', 'run', 'confirm', 'approve', 'yes', 'ok', 'continue', 'proceed'];
                    const hasAcceptText = acceptPatterns.some(p => text.includes(p) || ariaLabel.includes(p));
